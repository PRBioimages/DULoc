...
parser.add_argument('--num_classes', default=29, type=int, help='number of classes (default: 28)')
...
def main():
    # initial
    model_out_dir = '/home/mqxue/bestfitting/models/model_1024'
    log_out_dir = '/home/mqxue/bestfitting/result/logs/training_log'
    DATA_DIR = '/home/mqxue/bestfitting/HPA_imgs/Train_dataset'
    train_split_file = opj(DATA_DIR, 'train.csv')
    valid_split_file = opj(DATA_DIR, 'valid.csv')
    train_img_dir = opj(DATA_DIR, 'train')
    ...
    # log_out_dir = opj(RESULT_DIR, 'logs', out_dir, 'with_vesicles')
    if not ope(log_out_dir):
        os.makedirs(log_out_dir)
    log = Logger()
    log.open(opj(log_out_dir, 'log.train.txt'), mode='a')

    # model_out_dir = opj(RESULT_DIR, 'models', out_dir, 'with_vesicles')
    ...
    cudnn.benchmark = False # centos is true
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        iter, train_loss, train_acc = train(train_loader, model, criterion, optimizer, epoch, clipnorm=clipnorm,
                                            lr=lr)

        with torch.no_grad():
            valid_loss, valid_acc, valid_focal_loss, kaggle_score = validate(valid_loader, model, criterion, epoch,
                                                                             focal_loss)
