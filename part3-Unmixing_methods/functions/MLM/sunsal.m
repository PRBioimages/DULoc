function [z,res_p,res_d,i] = sunsal(M,y,varargin)
...
% work on the subspace coordinates
% if p < L
%    U = orth(M);
%    M = U'*M;
%    y = U'*y;
...
if  sum(sum(lambda == 0)) && strcmp(addone,'yes') && strcmp(positivity,'no')
    F = M'*M;
...
if x0 == 0
%    x = IF*M'*y;
    x = (M'*M)\M'*y; %% 25/08/2015 Ignore the regularization
%     x = UF*diag(1./sF)*UF'*yy;
%     x = pinv(M)*y;
else
    x = x0;
end
...
%--------------------------------------------------------------------------
if sum(lambda(:) ==  0)  && strcmp(addone,'no')
    ...
    %--------------------------------------------------------------------------
elseif sum(lambda(:) ==  0)  && strcmp(addone,'yes')
    while (i <= AL_iters) && ((abs (res_p) > tol1) || (abs (res_d) > tol2)) %&& norm(X_sol-z,'fro') >conv_thre
        % save z to be used later
        if mod(i,10) == 1
            z0 = z;
        end
...
%         z_err_set=[z_err_set(2:end) norm(X_sol-z,'fro')];
%         if mean(z_err_set) < conv_thre  %&& norm(X_sol-z,'fro')/c_norm < conv_thre        
%             break;
%         end
    end
    %--------------------------------------------------------------------------
